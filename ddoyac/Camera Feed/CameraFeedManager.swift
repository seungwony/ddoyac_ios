...
//    pixelBuffer?.resized(to: CGSize(width: 1024, height: 1024))
    ...
    delegate?.didOutput(sampleBuffer: sampleBuffer, pixelBuffer: imagePixelBuffer)
    
 
  }
    
    func resize(_ imageBuffer : CVPixelBuffer, _ destSize: CGSize)-> CVPixelBuffer? {
//        guard let imageBuffer = CMSampleBufferGetImageBuffer(self as! CMSampleBuffer) else { return nil }
